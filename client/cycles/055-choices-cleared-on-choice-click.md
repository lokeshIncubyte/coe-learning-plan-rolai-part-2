...
- **Assertion**:
  ...
  ```
- **Why it fails**: After cycle-053's GREEN, `handleChoice` is a stub `(_label) => {}` that does nothing — `dispatch` is never called, so `choices` stays non-empty in state and the `ChoiceList` remains rendered.
