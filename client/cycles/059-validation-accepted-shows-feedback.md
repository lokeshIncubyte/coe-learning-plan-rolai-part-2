...
- **Assertion**:
  ...
  ```
- **Why it fails**: After cycle-058's GREEN, the page handles `rejected: true` but never sets any validation status state — `ValidationFeedback` is not rendered at all, so `data-testid="feedback-indicator"` does not exist in the DOM.
