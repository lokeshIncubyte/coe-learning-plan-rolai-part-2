...
- **Assertion**:
  ...
  ```
- **Why it fails**: After web-057's GREEN, `handleSubmit` calls `start({ prompt: text })` unconditionally (the existing fetch in `useStream` itself is for the stream endpoint, not validation). There is no `POST /api/generate` validation call in `handleSubmit`, so `mockStart` is always called regardless of what `fetch` returns.
