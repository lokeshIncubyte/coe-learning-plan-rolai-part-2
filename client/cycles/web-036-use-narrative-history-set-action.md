...
- **Assertion** (append to the existing test file — do NOT recreate it; imports are already present):
  ...
  ```
- **Why it fails**: `setChosenAction` is not returned by `useNarrativeHistory` — web-035's GREEN explicitly omits it, so calling `result.current.setChosenAction(...)` throws `TypeError: result.current.setChosenAction is not a function`.
