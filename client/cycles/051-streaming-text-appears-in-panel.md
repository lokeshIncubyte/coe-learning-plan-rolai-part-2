...
- **Assertion**:
  ...
  ```
- **Why it fails**: After cycle-050's GREEN, `StreamingText` is not rendered inside the narrative panel, so the panel has no text content and the assertion fails.

## GREEN
- **Smallest change**: Import `StreamingText` from `./components/StreamingText`. Inside the `data-testid="narrative-panel"` div, render `<StreamingText text={narrativeText} isStreaming={isStreaming} />` unconditionally (gating is added in cycle-052). `narrativeText` and `isStreaming` come from `useStreamState` and `useStream` respectively.
