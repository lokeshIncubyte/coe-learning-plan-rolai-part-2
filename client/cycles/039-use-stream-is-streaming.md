...
## GREEN
- **Smallest change**: Create `app/narrative/hooks/useStream.ts`. `start()` sets `isStreaming = true` and fires a fetch but does NOT process the response body (that is added in cycle-040):
  ```ts
...
- The `fetch(...)` call is unawaited (fire-and-forget). TypeScript does not warn about floating promises without an explicit ESLint rule (`@typescript-eslint/no-floating-promises`), which is not enforced during `jest`. This is intentional — body reading is added in cycle-040.
