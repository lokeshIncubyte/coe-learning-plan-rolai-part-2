...
- **Assertion**:
  ```ts
  // Mock setup for useNarrativeHistory is already in place from web-056.
  ...
  ```
- **Why it fails**: After web-056's GREEN, `addBeat` is never called in the `onEvent` handler — the `done` event only dispatches to `useStreamState`. `narrativeHistoryState.addBeat` remains uncalled and the assertion fails.
