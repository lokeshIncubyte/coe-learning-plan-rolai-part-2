...
**Owning cycle:** `cycle-066` (`lore-upload-service-log-deltas`)
...
**Owning cycle:** `cycle-062` (`extractor-service-apply-new-edge`)
...
**Owning cycles:** `cycle-058` (defines `Delta` union) + `cycle-064` (`extractor-service-apply-citation`)
...
**Owning cycle:** `cycle-056` (`lore-upload-service-process-upload-pdf`)
...
**Owning cycle:** `cycle-056` (missing sibling)
...
**Owning cycle:** `cycle-068` (`upload-controller-post`)
...
|---|-----|-------------|----------|
| 1 | `sessionId: 'upload'` FK violation | cycle-066 | (d) db-constraint-not-modeled + (a) mock-hiding-contract |
| 2 | LLM hallucinated edge endpoints | cycle-062 | (b) missing-error-path-cycle |
| 3 | `source?` missing from `NewEntityDelta` | cycle-058 + cycle-064 | (c) external-contract-not-read + (e) type-not-validated-at-boundary |
| 4 | `pdf-parse` v2 API mismatch | cycle-056 | (a) mock-hiding-contract + (c) external-contract-not-read |
| 5 | Malformed PDF → 500 | cycle-056 (missing sibling) | (b) missing-error-path-cycle |
| 6 | `@types/multer` missing | cycle-068 | (e) type-not-validated-at-boundary |
