...
- **Assertion**: No unit assertion. Without this schema change, the Prisma-generated `Entity` type lacks `archetype`, `identity_version`, and `embedding`, so `EmbeddingService` (cycle-029 onward) would fail TypeScript compilation when accessing those fields.
