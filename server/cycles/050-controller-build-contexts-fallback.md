...
- **Assertion**:
  ...
  ```
- **Why it fails**: after cycle-049's GREEN, `buildContexts` calls `semanticRecall` but has no fallback — when it returns empty entities the controller proceeds with an empty world context instead of calling `getAllEntitiesWithEdges`.
