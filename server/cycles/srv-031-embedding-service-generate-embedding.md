...
## Behavior
`EmbeddingService.generateEmbedding(text)` calls the local ONNX embedding proxy (`helper-apis`) via the OpenAI SDK and returns a 384-element `number[]`. This cycle covers the happy path only — the error/fallback path is covered in srv-051.
