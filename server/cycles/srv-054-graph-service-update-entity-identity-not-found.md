...
- **Assertion** (add inside the existing `describe('updateEntityIdentity')` block, after the happy-path test):
  ...
  ```
- **Why it fails**: After srv-038's GREEN, `updateEntityIdentity` calls `prisma.entity.update` without checking whether `findUnique` returned null — Prisma throws its own error (record not found) rather than a NestJS `NotFoundException`.
