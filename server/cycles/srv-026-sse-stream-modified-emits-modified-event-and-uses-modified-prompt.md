...
## GREEN
- After the rejected guard added in srv-025, add:
  ```ts
...
- **Pre-condition**: The existing `describe('stream SSE endpoint')` `beforeEach` must have been updated per srv-025's existing-test fix (validate mocked to `mockResolvedValue({ result: 'accepted' })`) before this cycle's GREEN is applied.
