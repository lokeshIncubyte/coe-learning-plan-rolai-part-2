...
## GREEN
- **Smallest change**: After building the `results` array in `evaluateRules`, add `.sort((a, b) => b.priority !== a.priority ? b.priority - a.priority : b.specificity - a.specificity)`. Ensure each pushed `RuleResult` sets `specificity: facts.triggers.length`. The AND logic is already correct from cycle-043 (`facts.triggers.every(...)`); verify it is in place.
