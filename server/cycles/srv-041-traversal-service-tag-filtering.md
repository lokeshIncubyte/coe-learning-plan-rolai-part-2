...
- **Assertion** (add inside the `describe('TraversalService')` block, after the BFS core tests):
  ...
  ```
- **Why it fails**: The current `traverse` implementation from srv-040 ignores the `tags` parameter entirely — it follows all edges and therefore returns both `'b'` and `'c'` even when only `['combat']` edges should be traversed, causing the first assertion to fail.
