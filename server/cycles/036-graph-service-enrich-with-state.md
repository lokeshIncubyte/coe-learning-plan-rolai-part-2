...
## Behavior
`GraphService.enrichWithState(ids)` is the Phase 2 graph-layer fetch. It retrieves entities by ID list from Prisma, including their `fromEdges` and `toEdges` relations. It preserves the Phase 1 ordering — if IDs arrive in similarity order, the returned array is in that same order even when the database returns rows in a different sequence. The empty-ids guard (returning `[]` without hitting the database) is covered in cycle-052.
